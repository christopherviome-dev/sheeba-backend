...
const MAX_PHOTO_CHARS = 1.5 * 1024 * 1024; // ~1.1 MB image; the app resizes ID photos to well under this (see lib/validate.js for the storage budget)
...
// Other ID documents (passport, driving licence, residence permit): formats
// vary too much to check strictly, so just letters and digits, 5-20 long,
// ignoring spaces and dashes. The admin still compares it with the photo.
function normalizeIdNumber(raw) {
  if (typeof raw !== 'string') return null;
  const n = raw.toUpperCase().replace(/[\s-]/g, '');
  return /^[A-Z0-9]{5,20}$/.test(n) ? n : null;
}

module.exports = { normalizeIdNumber, normalizeGhanaCard, cleanLegalName, compareNames, nameTokens, checkCardPhoto };
